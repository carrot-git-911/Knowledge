...
var OverrideExtensionModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(contentEl).setName("附件存放根路径").setDesc("选择附件存放的根路径").addDropdown(
    ...
    if (this.settings.saveAttE !== "obsFolder") {
      new import_obsidian2.Setting(contentEl).setName("根路径").setClass("override_root_folder_set").addText(
    ...
    new import_obsidian2.Setting(contentEl).setName("附件路径").setDesc(
      `根文件夹中的附件路径，可用变量 ${SETTINGS_VARIABLES_NOTEPATH}, ${SETTINGS_VARIABLES_NOTENAME} 和${SETTINGS_VARIABLES_NOTEPARENT}`
    ).addText(
    ...
    new import_obsidian2.Setting(contentEl).setName("附件命名格式").setDesc(
      `定义如何命名附件文件，可用变量 ${SETTINGS_VARIABLES_DATES}, ${SETTINGS_VARIABLES_NOTENAME}, ${SETTINGS_VARIABLES_MD5} 和 ${SETTINGS_VARIABLES_ORIGINALNAME}.`
    ).addText(
...
var SettingTab = class extends import_obsidian3.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian3.Setting(containerEl).setName("附件存放根路径").setDesc("选择附件存放的根路径").addDropdown(
    ...
    new import_obsidian3.Setting(containerEl).setName("根路径").setDesc("新附件的根文件夹").setClass("root_folder_set").addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("附件路径").setDesc(
      `根文件夹中的附件路径，可用变量 ${SETTINGS_VARIABLES_NOTEPATH}, ${SETTINGS_VARIABLES_NOTENAME}, ${SETTINGS_VARIABLES_NOTEPARENT}`
    ).addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("附件命名格式").setDesc(
      `定义如何命名附件文件，可用变量 ${SETTINGS_VARIABLES_DATES}, ${SETTINGS_VARIABLES_NOTENAME}, ${SETTINGS_VARIABLES_MD5} 和 ${SETTINGS_VARIABLES_ORIGINALNAME}.`
    ).addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("日期格式").setDesc(
      createFragment((frag) => {
        frag.appendText("时间格式 ");
        frag.createEl("a", {
    ...
    new import_obsidian3.Setting(containerEl).setName("自动重命名附件").setDesc(
      "当您重命名相应的md/canvas文件所在的文件夹/文件名时，自动重命名附件。"
    ).addToggle(
    ...
    new import_obsidian3.Setting(containerEl).setName("自定义重命名规则").setDesc("根据文件类型不同，使用您指定的命名规则来自动重命名附件（例如.pdf或zip）。").addButton((btn) => {
    ...
    if (this.plugin.settings.attachPath.extensionOverride !== void 0) {
      this.plugin.settings.attachPath.extensionOverride.forEach((ext) => {
        new import_obsidian3.Setting(containerEl).setName("自定义规则").setDesc("扩展的重命名规则").setClass("override_extension_set").addText(
          (text) => text.setPlaceholder("pdf|docx?").setValue(ext.extension).onChange(async (value) => {
            ext.extension = value;
          })
        ).addButton((btn) => {
          btn.setIcon("trash").setTooltip("移除自定义").onClick(async () => {
            var _a, _b, _c;
          ...
          btn.setIcon("pencil").setTooltip("编辑自定义").onClick(async () => {
            new OverrideExtensionModal(this.plugin, ext, (result) => {
              ext = result;
            }).open();
          });
        }).addButton((btn) => {
          btn.setIcon("check").setTooltip("保存自定义").onClick(async () => {
    ...
    new import_obsidian3.Setting(containerEl).setName("排除指定格式的文件").setDesc(`使用正则表达式筛选需要排除在处理之外的文件类型。`).addText(
    ...
    new import_obsidian3.Setting(containerEl).setName("排除指定路径").setDesc(
      `提供需要排除重命名的文件夹名称（区分大小写，不带前导斜杠“/”），并用分号（;）分隔。`
    ...
    new import_obsidian3.Setting(containerEl).setName("排除子路径").setDesc("如果你想同时排除上面提供的文件夹路径的所有子文件夹，请打开此选项。").addToggle(
...
var OverrideModal = class extends import_obsidian4.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian4.Setting(contentEl).setName("附件存放根路径").setDesc("选择附件存放的根路径").addDropdown(
    ...
    new import_obsidian4.Setting(contentEl).setName("根路径").setClass("override_root_folder_set").addText(
    ...
    new import_obsidian4.Setting(contentEl).setName("附件路径").setDesc(
      `根文件夹中的附件路径，可用变量 ${SETTINGS_VARIABLES_NOTEPATH}, ${SETTINGS_VARIABLES_NOTENAME} 和${SETTINGS_VARIABLES_NOTEPARENT}`
    ).addText(
    ...
    new import_obsidian4.Setting(contentEl).setName("附件命名格式").setDesc(
      `定义如何命名附件文件，可用变量 ${SETTINGS_VARIABLES_DATES}, ${SETTINGS_VARIABLES_NOTENAME}, ${SETTINGS_VARIABLES_MD5} 和 ${SETTINGS_VARIABLES_ORIGINALNAME}.`
    ).addText(
    ...
    if (this.setting.extensionOverride !== void 0) {
      this.setting.extensionOverride.forEach((ext) => {
        new import_obsidian4.Setting(contentEl).setName("自定义规则").setDesc("扩展的重命名规则").setClass("override_extension_set").addText(
...
var ConfirmModal = class extends import_obsidian9.Modal {
  ...
  onOpen() {
    ...
    contentEl.createSpan("", (el) => {
      el.innerText = "此操作不可逆且为实验性质。请先备份您的配置！";
    });
